...
local function item_result_width(recipe)
  ...
  for _, result in ipairs((recipe and recipe.results) or {}) do
    if (result.type or "item") == "item" and result.name then
      local final_name = data_table_reader.final_result_name(result.name)
...
local function update_furnace_result_inventory_sizes(data_table)
  ...
  data_table_writer.set_furnace_result_inventory_audit(data_table, {
...
---Returns true when a generated technology unlocks the generated recipe.
---@param data_table ISdata_table
---@param recipe_name string
---@return boolean
local function generated_technology_unlocks_recipe(data_table, recipe_name)
  for _, tech in pairs(data_table_reader.generated_technologies(data_table) or {}) do
...
function patcher.validate_generated_prototypes(data_table)
  ...
  for name, item in pairs(data_table_reader.generated_items(data_table) or {}) do
    local source = data_table_reader.generated_source(data_table, "items", name)
  ...
  for name, recipe in pairs(data_table_reader.generated_recipes(data_table) or {}) do
    local source = data_table_reader.generated_source(data_table, "recipes", name)
    if not generated_technology_unlocks_recipe(data_table, name) then
  ...
  for name, tech in pairs(data_table_reader.generated_technologies(data_table) or {}) do
...
function patcher.patch_recycle_amounts(data_table)
  ...
  for _, insert in pairs(data_table_reader.recipe_inserts(data_table)) do
  ...
  for scrap_name, total in pairs(totals) do
    ...
    for _, recipe_name in ipairs(recipe_names) do
      if data_table_writer.set_generated_recipe_input_amount(data_table, recipe_name, needed) then
...
---Applies registered machine-category patch rules to the final prototype set.
---@param data_table ISdata_table
local function patch_machine_categories(data_table)
...
function patcher.patch(data_table)
  ...
  if #items_to_extend > 0 then
    ...
    log("Registered " .. #items_to_extend .. " material type(s).")
  end

  local recipes_to_extend = {}
  for _, recipe_proto in pairs(data_table_reader.generated_recipes(data_table)) do
  ...
  for _, tech_proto in pairs(data_table_reader.generated_technologies(data_table)) do
  ...
  for recipe_name, insert_data in pairs(data_table_reader.recipe_inserts(data_table)) do
    ...
    if recipe and insert_data.results and not is_recycling_recipe then
      ...
      local has_mixed_result = false
      for _, result in ipairs(insert_data.results) do
        local final_name = data_table_reader.final_result_name(result.name)
        if final_name == "yis-mixed-scrap" then has_mixed_result = true end
      ...
      inserts = inserts + 1
      if has_mixed_result then mixed_inserts = mixed_inserts + 1 end
    end
  end
