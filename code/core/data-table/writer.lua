...
---Finds, stores, and returns a recipe main product, falling back to the first result.
---@param data_table ISdata_table
---@param recipe ISRecipePrototype
---@return string|nil
function writer.set_main_product(data_table, recipe)
  ...
  insert.main_product = main_product
  return main_product
end
...
function writer.cleanup_recipe_collection(data_table, recipe_name)
  ...
  if insert and (not insert.main_product or not insert.results) then
