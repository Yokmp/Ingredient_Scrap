...
overrides.prototype_alias_order = {
  item = {},
  fluid = {},
}
...
---Adds a prototype alias to the ordered alias list only once.
---@param prototype_type "item"|"fluid"
---@param material_name string
---@param prototype_name string
local function add_ordered_alias(prototype_type, material_name, prototype_name)
  local values = overrides.prototype_alias_order[prototype_type][material_name] or {}
  overrides.prototype_alias_order[prototype_type][material_name] = values
  for _, value in ipairs(values) do
    if value == prototype_name then return end
  end
  table.insert(values, prototype_name)
end
...
local function register_aliases(prototype_type, aliases, material_name)
  for _, prototype_name in ipairs(aliases or {}) do
    if type(prototype_name) == "string" and prototype_name ~= "" then
      ...
      add_ordered_alias(prototype_type, material_name, prototype_name)
    end
  end
end

---Normalizes one settings icon definition.
---@param icon table|string|nil
---@return table|nil
local function normalize_setting_icon(icon)
  if type(icon) == "string" and icon ~= "" then
    return { type = "item", name = icon }
  end
  if type(icon) == "table" and (icon.type == "item" or icon.type == "fluid") and type(icon.name) == "string" and icon.name ~= "" then
    return { type = icon.type, name = icon.name }
  end
  return nil
end

---Adds one settings icon candidate for a material.
---@param material_name string
---@param icon table|string|nil
local function add_setting_icon(material_name, icon)
  local normalized = normalize_setting_icon(icon)
  if not normalized then return end
  overrides.setting_icons[material_name] = overrides.setting_icons[material_name] or {}
  for _, value in ipairs(overrides.setting_icons[material_name]) do
    if value.type == normalized.type and value.name == normalized.name then return end
  end
  table.insert(overrides.setting_icons[material_name], normalized)
end

---Adds settings icon candidates for a material.
---@param material_name string
---@param icon table|string|nil
---@param icons table[]|nil
local function register_setting_icons(material_name, icon, icons)
  add_setting_icon(material_name, icon)
  for _, value in ipairs(icons or {}) do
    add_setting_icon(material_name, value)
  end
end
...
---@param definition {name: string, default?: string, prototype_affixes?: table, prototype_aliases?: table, setting_icon?: table|string, setting_icons?: table[], localized_setting_name?: boolean, source?: string|table, tint?: table|string, exact_scrap?: boolean}
function overrides.register_material_override(definition)
  ...
  register_setting_icons(definition.name, definition.setting_icon, definition.setting_icons)
...
local function material_definition(name, mode, options)
  ...
  return {
    ...
    setting_icon = options.setting_icon,
...
local function publish_material_api()
  ...
  api.register.material.alias = function(name, prototype_type, prototype_name)
    if type(name) == "string" and name ~= "" and
      ...
      add_ordered_alias(prototype_type, name, prototype_name)
    end
  end
  api.register.material.setting_icon = function(name, icon)
    if type(name) == "string" and name ~= "" then
      add_setting_icon(name, icon)
    end
  end
...
---Returns the settings-stage rich text icon for a material.
---@param material_name string
---@return string
function overrides.setting_icon_tag(material_name)
  for _, icon in ipairs(overrides.setting_icons[material_name] or {}) do
    return "[" .. icon.type .. "=" .. icon.name .. "]"
  end

  for _, item_name in ipairs(overrides.prototype_alias_order.item[material_name] or {}) do
    return "[item=" .. item_name .. "]"
  end

  for _, fluid_name in ipairs(overrides.prototype_alias_order.fluid[material_name] or {}) do
    return "[fluid=" .. fluid_name .. "]"
  end
...
function overrides.localised_setting_name(material_name)
  local material_label = material_name
  if overrides.localized_setting_names[material_name] then
    material_label = { "mod-setting-name." .. overrides.setting_name(material_name) }
  end
  return { "", overrides.setting_icon_tag(material_name), " - ", material_label, ": ", { "mod-setting-name.yis-material-mode" } }
end
