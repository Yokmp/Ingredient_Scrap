...
The original blocker was to compare the old component/static behavior against
the ancestry resolver before the first public release. This is done. The active
resolver is now the iterative ancestry/mixed-scrap resolver, and generated
prototype names use the `yis-` prefix consistently.

Before publishing, only release hygiene remains:

- keep `README.md`, `docs/API.md`, and `docs/DEV.md` in sync with the shipped behavior;
- keep the Mod Portal zip free of Markdown, screenshots, tools, dumps, and
  debug-only regions;
- keep `public.zip` as the GitHub `main` source archive with documentation;
- run the standard test matrix after release-critical changes.

### Next

1. Release checklist:
   - 1.1. Done: split documentation into `README.md`, `docs/API.md`, and `docs/DEV.md`.
   - 1.2. Done: keep `changelog.txt` short and point detailed notes to GitHub.
   - 1.3. Done: verify Mod Portal zip contents and `public.zip` contents.
   - 1.4. Done: run `python tools/test/run_tests.py --all --no-color`.
2. Remaining before publish:
   - 2.1. Optionally run targeted K2/Bob/Angels profiles again if their local
     mod set changed after the last test pass.
   - 2.2. Start Factorio from the final Mod Portal zip once after the last build.
   - 2.3. Publish `public.zip` to GitHub `main` only after reviewing the final
     generated source archive.
3. Future-only work:
   - Preserve Recipe Shape.
   - Chemistry/Metallurgy as a dedicated extension mod.
   - Schrotthaufen/resource-source gameplay.
   - Viewer polish, profiler experiments, and large API extensions only when a
     real compat gap requires them.
...
Status: done. The data table is private core state owned by the internal API and
the `code/data_table` reader/writer helpers.

Rules:

- `code/data_table/init.lua` owns the initial table shape.
- `code/data_table/writer.lua` owns semantically important writes.
- `code/data_table/reader.lua` owns shared read access.
- Active resolver and patcher code should go through the internal API/context
  instead of mutating global data-table state directly.
- Debug dumps may still read snapshots for reporting, but should not mutate
  operational state.

Acceptance checks:

- No active `code.core.data-table-writer` require paths remain.
...
Operational writes that should use `code/data_table/writer.lua`:
...
- If a resolver wants to become active, it must write the same prototype and
  insert shape through `code/data_table/writer.lua`.
...
Historical goal: prevent passive planning work from sliding into active resolver
implementation without an explicit reasoning and user-confirmation checkpoint.

Status: done. Active ancestry is now implemented under `code/resolver/ancestry`
and is the release behavior. The original guard is retained here only as a
process note for future large resolver rewrites.

For future resolver rewrites, stop before implementing any equivalent large
behavior switch:

- creating a new active resolver module family;
- running new resolver logic in the data stage;
- writing resolver-derived results to operational recipe inserts;
- creating new fallback scrap prototypes or recycle recipes;
- changing patcher behavior to understand resolver-specific data.
...
3. Re-read the latest active roadmap and historical resolver notes.
4. Start with passive runtime output only. Active patching comes later and only
   after passive runtime summaries match archived profiles.
...
- local planning/repo files such as `docs/DESIGN_NOTES.md`, `.gitignore`, and
  `.gitattributes`;
- `locale/en/test.cfg`.
...
1. Implement `Preserve recipe shape` in small tested steps. Keep this behind the
...
2. Consider an `Advanced scrap recycling` chain. This should be separate from
...
3. Split test/report severities so review output can distinguish:
...
4. Keep polishing the local tool UI and JSON viewer as development helpers.
5. Build a Python weight/evidence tuner only after there is a real golden-table
   set to optimize against.
...
5. Compat/API:
   - Material override API, category API, source-filter API, recipe-chain target
     API, nested public wrappers, typed patch queue, generated snapshots, and
     dedicated `docs/API.md` documentation.
...
8. Internal structure:
   - `code/api`, `code/data_table`, `code/resolver`, `code/patcher`,
     `code/override`, and `code/functions` separate the public API, internal
     state, resolver, patcher, override registries, and shared helpers.
   - `data_table` is treated as private core state and should be read/written
     through the internal API and data-table reader/writer helpers.
9. Ancestry research:
   - The passive ancestry comparison informed the current active resolver.
   - The active iterative ancestry/mixed-scrap resolver is now the release
     behavior.
10. Release/documentation:
   - `README.md` is player-facing, `docs/API.md` is for compat authors, and `docs/DEV.md`
     covers test harness, debug dumps, tools, and deploy workflow.
   - `Ingredient_Scrap_<version>.zip` is the Mod Portal artifact without
     Markdown, screenshots, tools, dumps, or debug-only regions.
   - `public.zip` is the GitHub `main` source archive and includes documentation.
...
Resolver decisions should log through `code.functions.is-log` so
...
## Historical: Ancestry-Based Scrap Solver

The most promising long-term model was to stop treating every matched
...
This has now become the active release direction. The notes below are retained
as historical design context for future resolver work and regression review.
...
`code/data_table/init.lua` creates the shared plain data table. The internal API
context owns that state and exposes controlled reader/writer operations to the
resolver, patcher, debug, and tests.

Current stage split:

- `data.lua`: expose the public API, register compatibility rules, create the
  internal context and base recipe categories.
- `data-updates.lua`: release placeholder only; debug timing may live here
  inside stripped debug regions.
- `data-final-fixes.lua`: run resolver, queue processing, patching, generated
  prototype validation/extend, and debug report staging.

`code/data_table/writer.lua` is the internal abstraction layer for semantically
important writes. It keeps the table itself plain and dump-friendly, while
centralizing fragile operations such as:
...
`get_main_product()` are no longer public API. Internal equivalents delegate to
this
writer. The active data-stage resolver entrypoint is
`data-final-fixes.lua`: it runs material resolution, active ancestry lookup,
mixed-scrap finalization, typed queue processing, patcher validation, and
`data:extend` through the internal API/context. Patcher, debug dumps, and
analysis code may still iterate over plain snapshot tables directly when they
only need broad read access.
