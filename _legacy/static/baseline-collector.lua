...
local item_prototypes = require("code.lib.item-prototypes")
...
local function scrap_source_item_for_solid(scrap_type, ingredient_name)
  ...
  for _, preferred_name in ipairs(preferred_names) do
    local prototype = item_prototypes.get(preferred_name)
    if prototype then
      return preferred_name, prototype
    end
  end

  local ingredient_item = item_prototypes.get(ingredient_name)
  return ingredient_name, ingredient_item
end
...
local function scrap_source_item_for_fluid(scrap_type, main_product)
  local main_product_item = item_prototypes.get(main_product)
  if main_product_item then
    return main_product, main_product_item
  end
  ...
  for _, fallback_name in ipairs(fallback_names) do
    local fallback_item = item_prototypes.get(fallback_name)
