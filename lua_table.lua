-- Extrahiert aus: ../../factorio-current.log
return {
  constants = {
    icon_path = "__Ingredient_Scrap__/graphics/icons/",
    icon_scrap = {
      "scrap"
    },
    recycle_categories = {
      fluid = "yis-recycle_to_fluid",
      solid = "yis-recycle-to-item"
    },
    scrap_pictures = 3
  },
  ingredients = {
    fluids = {},
    items = {
      accumulator = {
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      ["advanced-circuit"] = {
        {
          amount = 4,
          name = "copper-cable",
          type = "item"
        }
      },
      ["advanced-thruster-oxidizer"] = {
        {
          amount = 2,
          name = "iron-ore",
          type = "item"
        }
      },
      ["agricultural-tower"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["arithmetic-combinator"] = {
        {
          amount = 5,
          name = "copper-cable",
          type = "item"
        }
      },
      ["artillery-shell"] = {
        {
          amount = 4,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["artillery-turret"] = {
        {
          amount = 60,
          name = "tungsten-plate",
          type = "item"
        },
        {
          amount = 40,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["artillery-wagon"] = {
        {
          amount = 60,
          name = "tungsten-plate",
          type = "item"
        },
        {
          amount = 40,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["assembling-machine-1"] = {
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 9,
          name = "iron-plate",
          type = "item"
        }
      },
      ["assembling-machine-2"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["automation-science-pack"] = {
        {
          amount = 1,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      barrel = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      battery = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "copper-plate",
          type = "item"
        }
      },
      ["battery-equipment"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      beacon = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "copper-cable",
          type = "item"
        }
      },
      ["belt-immunity-equipment"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["big-electric-pole"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 4,
          name = "copper-cable",
          type = "item"
        }
      },
      biochamber = {
        {
          amount = 20,
          name = "iron-plate",
          type = "item"
        }
      },
      ["bulk-inserter"] = {
        {
          amount = 15,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["burner-inserter"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["burner-mining-drill"] = {
        {
          amount = 3,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 3,
          name = "iron-plate",
          type = "item"
        }
      },
      ["cannon-shell"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      ["capture-robot-rocket"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      car = {
        {
          amount = 20,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        }
      },
      ["cargo-bay"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["cargo-landing-pad"] = {
        {
          amount = 25,
          name = "steel-plate",
          type = "item"
        }
      },
      ["cargo-wagon"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 20,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      centrifuge = {
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 100,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["chemical-plant"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["cluster-grenade"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        }
      },
      ["combat-shotgun"] = {
        {
          amount = 15,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "copper-plate",
          type = "item"
        }
      },
      concrete = {
        {
          amount = 1,
          name = "iron-ore",
          type = "item"
        }
      },
      ["constant-combinator"] = {
        {
          amount = 5,
          name = "copper-cable",
          type = "item"
        }
      },
      ["copper-cable"] = {
        {
          amount = 1,
          name = "copper-plate",
          type = "item"
        }
      },
      ["copper-cable-recycling"] = {
        {
          amount = 1,
          name = "copper-cable",
          type = "item"
        }
      },
      ["copper-ore-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "copper-ore",
          type = "item"
        }
      },
      ["copper-plate"] = {
        {
          amount = 1,
          name = "copper-ore",
          type = "item"
        }
      },
      ["copper-plate-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "copper-plate",
          type = "item"
        }
      },
      crusher = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["decider-combinator"] = {
        {
          amount = 5,
          name = "copper-cable",
          type = "item"
        }
      },
      ["defender-capsule"] = {
        {
          amount = 3,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["destroyer-capsule"] = {
        {
          amount = 4,
          name = "steel-plate",
          type = "item"
        }
      },
      ["discharge-defense-equipment"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["display-panel"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["electric-furnace"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["electric-mining-drill"] = {
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      },
      ["electromagnetic-plant"] = {
        {
          amount = 150,
          name = "holmium-plate",
          type = "item"
        },
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        }
      },
      ["electronic-circuit"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 3,
          name = "copper-cable",
          type = "item"
        }
      },
      ["energy-shield-equipment"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["engine-unit"] = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["exoskeleton-equipment"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["explosive-cannon-shell"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      ["express-splitter"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["express-transport-belt"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["express-underground-belt"] = {
        {
          amount = 80,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["fast-inserter"] = {
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      ["fast-splitter"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["fast-transport-belt"] = {
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["fast-underground-belt"] = {
        {
          amount = 40,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["firearm-magazine"] = {
        {
          amount = 4,
          name = "iron-plate",
          type = "item"
        }
      },
      flamethrower = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["flamethrower-ammo"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        }
      },
      ["flamethrower-turret"] = {
        {
          amount = 30,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 15,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["fluid-wagon"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 16,
          name = "steel-plate",
          type = "item"
        }
      },
      ["flying-robot-frame"] = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      foundation = {
        {
          amount = 4,
          name = "tungsten-plate",
          type = "item"
        }
      },
      foundry = {
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        }
      },
      ["fusion-generator"] = {
        {
          amount = 100,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["fusion-power-cell"] = {
        {
          amount = 1,
          name = "holmium-plate",
          type = "item"
        }
      },
      ["fusion-reactor"] = {
        {
          amount = 200,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["fusion-reactor-equipment"] = {
        {
          amount = 250,
          name = "tungsten-plate",
          type = "item"
        }
      },
      gate = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      grenade = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["gun-turret"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "iron-plate",
          type = "item"
        }
      },
      ["heat-exchanger"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 100,
          name = "copper-plate",
          type = "item"
        }
      },
      ["heat-pipe"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "copper-plate",
          type = "item"
        }
      },
      ["heavy-armor"] = {
        {
          amount = 100,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        }
      },
      ["holmium-ore-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "holmium-ore",
          type = "item"
        }
      },
      ["holmium-plate-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "holmium-plate",
          type = "item"
        }
      },
      ["holmium-solution"] = {
        {
          amount = 2,
          name = "holmium-ore",
          type = "item"
        }
      },
      inserter = {
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["iron-chest"] = {
        {
          amount = 8,
          name = "iron-plate",
          type = "item"
        }
      },
      ["iron-gear-wheel"] = {
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      ["iron-gear-wheel-recycling"] = {
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["iron-ore-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "iron-ore",
          type = "item"
        }
      },
      ["iron-plate"] = {
        {
          amount = 1,
          name = "iron-ore",
          type = "item"
        }
      },
      ["iron-plate-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["iron-stick"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      lab = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["land-mine"] = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      ["laser-turret"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["light-armor"] = {
        {
          amount = 40,
          name = "iron-plate",
          type = "item"
        }
      },
      ["lightning-rod"] = {
        {
          amount = 12,
          name = "copper-cable",
          type = "item"
        },
        {
          amount = 8,
          name = "steel-plate",
          type = "item"
        }
      },
      lithium = {
        {
          amount = 1,
          name = "holmium-plate",
          type = "item"
        }
      },
      loader = {
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      locomotive = {
        {
          amount = 30,
          name = "steel-plate",
          type = "item"
        }
      },
      ["long-handed-inserter"] = {
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["low-density-structure"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "copper-plate",
          type = "item"
        }
      },
      ["mech-armor"] = {
        {
          amount = 200,
          name = "holmium-plate",
          type = "item"
        }
      },
      ["medium-electric-pole"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 2,
          name = "copper-cable",
          type = "item"
        }
      },
      ["metallurgic-science-pack"] = {
        {
          amount = 2,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["modular-armor"] = {
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        }
      },
      ["molten-copper"] = {
        {
          amount = 50,
          name = "copper-ore",
          type = "item"
        }
      },
      ["molten-iron"] = {
        {
          amount = 50,
          name = "iron-ore",
          type = "item"
        }
      },
      ["night-vision-equipment"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["nuclear-reactor"] = {
        {
          amount = 500,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 500,
          name = "copper-plate",
          type = "item"
        }
      },
      ["offshore-pump"] = {
        {
          amount = 2,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["oil-refinery"] = {
        {
          amount = 15,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["personal-roboport-equipment"] = {
        {
          amount = 40,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["piercing-rounds-magazine"] = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 2,
          name = "copper-plate",
          type = "item"
        }
      },
      ["piercing-shotgun-shell"] = {
        {
          amount = 5,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      pipe = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["pipe-to-ground"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      pistol = {
        {
          amount = 5,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["poison-capsule"] = {
        {
          amount = 3,
          name = "steel-plate",
          type = "item"
        }
      },
      ["power-armor"] = {
        {
          amount = 40,
          name = "steel-plate",
          type = "item"
        }
      },
      ["power-switch"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "copper-cable",
          type = "item"
        }
      },
      ["programmable-speaker"] = {
        {
          amount = 3,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "copper-cable",
          type = "item"
        }
      },
      pump = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      pumpjack = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      radar = {
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      },
      rail = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      ["rail-chain-signal"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["rail-ramp"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["rail-signal"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["rail-support"] = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      railgun = {
        {
          amount = 10,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["railgun-ammo"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "copper-cable",
          type = "item"
        }
      },
      ["railgun-turret"] = {
        {
          amount = 30,
          name = "tungsten-plate",
          type = "item"
        }
      },
      recycler = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 40,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["refined-concrete"] = {
        {
          amount = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      ["repair-pack"] = {
        {
          amount = 2,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      roboport = {
        {
          amount = 45,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 45,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      rocket = {
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      ["rocket-launcher"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["rocket-silo"] = {
        {
          amount = 1000,
          name = "steel-plate",
          type = "item"
        }
      },
      ["rocket-turret"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      shotgun = {
        {
          amount = 15,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "copper-plate",
          type = "item"
        }
      },
      ["shotgun-shell"] = {
        {
          amount = 2,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      ["slowdown-capsule"] = {
        {
          amount = 2,
          name = "steel-plate",
          type = "item"
        }
      },
      ["small-electric-pole"] = {
        {
          amount = 2,
          name = "copper-cable",
          type = "item"
        }
      },
      ["small-lamp"] = {
        {
          amount = 3,
          name = "copper-cable",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      ["solar-panel"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "copper-plate",
          type = "item"
        }
      },
      ["solar-panel-equipment"] = {
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        }
      },
      ["space-platform-foundation"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 20,
          name = "copper-cable",
          type = "item"
        }
      },
      ["space-platform-starter-pack"] = {
        {
          amount = 20,
          name = "steel-plate",
          type = "item"
        }
      },
      ["space-science-pack"] = {
        {
          amount = 2,
          name = "iron-plate",
          type = "item"
        }
      },
      splitter = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["steam-engine"] = {
        {
          amount = 8,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      },
      ["steam-turbine"] = {
        {
          amount = 50,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 50,
          name = "copper-plate",
          type = "item"
        }
      },
      ["steel-chest"] = {
        {
          amount = 8,
          name = "steel-plate",
          type = "item"
        }
      },
      ["steel-furnace"] = {
        {
          amount = 6,
          name = "steel-plate",
          type = "item"
        }
      },
      ["steel-plate"] = {
        {
          amount = 5,
          name = "iron-plate",
          type = "item"
        }
      },
      ["steel-plate-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "steel-plate",
          type = "item"
        }
      },
      ["storage-tank"] = {
        {
          amount = 20,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 5,
          name = "steel-plate",
          type = "item"
        }
      },
      ["submachine-gun"] = {
        {
          amount = 10,
          name = "iron-gear-wheel",
          type = "item"
        },
        {
          amount = 5,
          name = "copper-plate",
          type = "item"
        },
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      },
      substation = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 6,
          name = "copper-cable",
          type = "item"
        }
      },
      ["sulfuric-acid"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        }
      },
      supercapacitor = {
        {
          amount = 2,
          name = "holmium-plate",
          type = "item"
        }
      },
      superconductor = {
        {
          amount = 1,
          name = "holmium-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "copper-plate",
          type = "item"
        }
      },
      tank = {
        {
          amount = 50,
          name = "steel-plate",
          type = "item"
        },
        {
          amount = 15,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      teslagun = {
        {
          amount = 10,
          name = "holmium-plate",
          type = "item"
        }
      },
      thruster = {
        {
          amount = 10,
          name = "steel-plate",
          type = "item"
        }
      },
      ["thruster-oxidizer"] = {
        {
          amount = 2,
          name = "iron-ore",
          type = "item"
        }
      },
      ["train-stop"] = {
        {
          amount = 6,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 3,
          name = "steel-plate",
          type = "item"
        }
      },
      ["transport-belt"] = {
        {
          amount = 1,
          name = "iron-plate",
          type = "item"
        },
        {
          amount = 1,
          name = "iron-gear-wheel",
          type = "item"
        }
      },
      ["tungsten-carbide"] = {
        {
          amount = 2,
          name = "tungsten-ore",
          type = "item"
        }
      },
      ["tungsten-ore-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "tungsten-ore",
          type = "item"
        }
      },
      ["tungsten-plate"] = {
        {
          amount = 4,
          name = "tungsten-ore",
          type = "item"
        }
      },
      ["tungsten-plate-recycling"] = {
        {
          amount = 1,
          ignored_by_stats = 1,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["turbo-splitter"] = {
        {
          amount = 15,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["turbo-transport-belt"] = {
        {
          amount = 5,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["turbo-underground-belt"] = {
        {
          amount = 40,
          name = "tungsten-plate",
          type = "item"
        }
      },
      ["underground-belt"] = {
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      },
      ["uranium-fuel-cell"] = {
        {
          amount = 10,
          name = "iron-plate",
          type = "item"
        }
      }
    }
  },
  inserts = {
    recipes = {
      accumulator = {
        main_product = "accumulator",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["advanced-circuit"] = {
        main_product = "advanced-circuit",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["advanced-thruster-oxidizer"] = {
        main_product = "thruster-oxidizer",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["agricultural-tower"] = {
        main_product = "agricultural-tower",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["arithmetic-combinator"] = {
        main_product = "arithmetic-combinator",
        results = {
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["artillery-shell"] = {
        main_product = "artillery-shell",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["artillery-turret"] = {
        main_product = "artillery-turret",
        results = {
          {
            amount = 5,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["artillery-wagon"] = {
        main_product = "artillery-wagon",
        results = {
          {
            amount = 5,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["assembling-machine-1"] = {
        main_product = "assembling-machine-1",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["assembling-machine-2"] = {
        main_product = "assembling-machine-2",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["automation-science-pack"] = {
        main_product = "automation-science-pack",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      barrel = {
        main_product = "barrel",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      battery = {
        main_product = "battery",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["battery-equipment"] = {
        main_product = "battery-equipment",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      beacon = {
        main_product = "beacon",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["belt-immunity-equipment"] = {
        main_product = "belt-immunity-equipment",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["big-electric-pole"] = {
        main_product = "big-electric-pole",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      biochamber = {
        main_product = "biochamber",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["bulk-inserter"] = {
        main_product = "bulk-inserter",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["burner-inserter"] = {
        main_product = "burner-inserter",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["burner-mining-drill"] = {
        main_product = "burner-mining-drill",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["cannon-shell"] = {
        main_product = "cannon-shell",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["capture-robot-rocket"] = {
        main_product = "capture-robot-rocket",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      car = {
        main_product = "car",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["cargo-bay"] = {
        main_product = "cargo-bay",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["cargo-landing-pad"] = {
        main_product = "cargo-landing-pad",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["cargo-wagon"] = {
        main_product = "cargo-wagon",
        results = {
          {
            amount = 7,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      centrifuge = {
        main_product = "centrifuge",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["chemical-plant"] = {
        main_product = "chemical-plant",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["cluster-grenade"] = {
        main_product = "cluster-grenade",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["combat-shotgun"] = {
        main_product = "combat-shotgun",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      concrete = {
        main_product = "concrete",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["constant-combinator"] = {
        main_product = "constant-combinator",
        results = {
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["copper-cable"] = {
        main_product = "copper-cable",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["copper-cable-recycling"] = {
        main_product = "copper-plate",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["copper-ore-recycling"] = {
        main_product = "copper-ore",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["copper-plate"] = {
        main_product = "copper-plate",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["copper-plate-recycling"] = {
        main_product = "copper-plate",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      crusher = {
        main_product = "crusher",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["decider-combinator"] = {
        main_product = "decider-combinator",
        results = {
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["defender-capsule"] = {
        main_product = "defender-capsule",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["destroyer-capsule"] = {
        main_product = "destroyer-capsule",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["discharge-defense-equipment"] = {
        main_product = "discharge-defense-equipment",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["display-panel"] = {
        main_product = "display-panel",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["electric-furnace"] = {
        main_product = "electric-furnace",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["electric-mining-drill"] = {
        main_product = "electric-mining-drill",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["electromagnetic-plant"] = {
        main_product = "electromagnetic-plant",
        results = {
          {
            amount = 6,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["electronic-circuit"] = {
        main_product = "electronic-circuit",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["energy-shield-equipment"] = {
        main_product = "energy-shield-equipment",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["engine-unit"] = {
        main_product = "engine-unit",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["exoskeleton-equipment"] = {
        main_product = "exoskeleton-equipment",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["explosive-cannon-shell"] = {
        main_product = "explosive-cannon-shell",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["express-splitter"] = {
        main_product = "express-splitter",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["express-transport-belt"] = {
        main_product = "express-transport-belt",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["express-underground-belt"] = {
        main_product = "express-underground-belt",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fast-inserter"] = {
        main_product = "fast-inserter",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fast-splitter"] = {
        main_product = "fast-splitter",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fast-transport-belt"] = {
        main_product = "fast-transport-belt",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fast-underground-belt"] = {
        main_product = "fast-underground-belt",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["firearm-magazine"] = {
        main_product = "firearm-magazine",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      flamethrower = {
        main_product = "flamethrower",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["flamethrower-ammo"] = {
        main_product = "flamethrower-ammo",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["flamethrower-turret"] = {
        main_product = "flamethrower-turret",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fluid-wagon"] = {
        main_product = "fluid-wagon",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["flying-robot-frame"] = {
        main_product = "flying-robot-frame",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      foundation = {
        main_product = "foundation",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      foundry = {
        main_product = "foundry",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fusion-generator"] = {
        main_product = "fusion-generator",
        results = {
          {
            amount = 5,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fusion-power-cell"] = {
        main_product = "fusion-power-cell",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fusion-reactor"] = {
        main_product = "fusion-reactor",
        results = {
          {
            amount = 6,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["fusion-reactor-equipment"] = {
        main_product = "fusion-reactor-equipment",
        results = {
          {
            amount = 7,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      gate = {
        main_product = "gate",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      grenade = {
        main_product = "grenade",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["gun-turret"] = {
        main_product = "gun-turret",
        results = {
          {
            amount = 7,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["heat-exchanger"] = {
        main_product = "heat-exchanger",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 5,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["heat-pipe"] = {
        main_product = "heat-pipe",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["heavy-armor"] = {
        main_product = "heavy-armor",
        results = {
          {
            amount = 5,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["holmium-ore-recycling"] = {
        main_product = "holmium-ore",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["holmium-plate-recycling"] = {
        main_product = "holmium-plate",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["holmium-solution"] = {
        main_product = "holmium-solution",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      inserter = {
        main_product = "inserter",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-chest"] = {
        main_product = "iron-chest",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-gear-wheel"] = {
        main_product = "iron-gear-wheel",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-gear-wheel-recycling"] = {
        main_product = "iron-plate",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-ore-recycling"] = {
        main_product = "iron-ore",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-plate"] = {
        main_product = "iron-plate",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-plate-recycling"] = {
        main_product = "iron-plate",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["iron-stick"] = {
        main_product = "iron-stick",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      lab = {
        main_product = "lab",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["land-mine"] = {
        main_product = "land-mine",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["laser-turret"] = {
        main_product = "laser-turret",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["light-armor"] = {
        main_product = "light-armor",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["lightning-rod"] = {
        main_product = "lightning-rod",
        results = {
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      lithium = {
        main_product = "lithium",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      loader = {
        main_product = "loader",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      locomotive = {
        main_product = "locomotive",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["long-handed-inserter"] = {
        main_product = "long-handed-inserter",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["low-density-structure"] = {
        main_product = "low-density-structure",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["mech-armor"] = {
        main_product = "mech-armor",
        results = {
          {
            amount = 6,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["medium-electric-pole"] = {
        main_product = "medium-electric-pole",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["metallurgic-science-pack"] = {
        main_product = "metallurgic-science-pack",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["modular-armor"] = {
        main_product = "modular-armor",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["molten-copper"] = {
        main_product = "molten-copper",
        results = {
          {
            amount = 4,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["molten-iron"] = {
        main_product = "molten-iron",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["night-vision-equipment"] = {
        main_product = "night-vision-equipment",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["nuclear-reactor"] = {
        main_product = "nuclear-reactor",
        results = {
          {
            amount = 8,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 8,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["offshore-pump"] = {
        main_product = "offshore-pump",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["oil-refinery"] = {
        main_product = "oil-refinery",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["personal-roboport-equipment"] = {
        main_product = "personal-roboport-equipment",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["piercing-rounds-magazine"] = {
        main_product = "piercing-rounds-magazine",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["piercing-shotgun-shell"] = {
        main_product = "piercing-shotgun-shell",
        results = {
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      pipe = {
        main_product = "pipe",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["pipe-to-ground"] = {
        main_product = "pipe-to-ground",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      pistol = {
        main_product = "pistol",
        results = {
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["poison-capsule"] = {
        main_product = "poison-capsule",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["power-armor"] = {
        main_product = "power-armor",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["power-switch"] = {
        main_product = "power-switch",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["programmable-speaker"] = {
        main_product = "programmable-speaker",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      pump = {
        main_product = "pump",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      pumpjack = {
        main_product = "pumpjack",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      radar = {
        main_product = "radar",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      rail = {
        main_product = "rail",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rail-chain-signal"] = {
        main_product = "rail-chain-signal",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rail-ramp"] = {
        main_product = "rail-ramp",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rail-signal"] = {
        main_product = "rail-signal",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rail-support"] = {
        main_product = "rail-support",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      railgun = {
        main_product = "railgun",
        results = {
          {
            amount = 3,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["railgun-ammo"] = {
        main_product = "railgun-ammo",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["railgun-turret"] = {
        main_product = "railgun-turret",
        results = {
          {
            amount = 4,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      recycler = {
        main_product = "recycler",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["refined-concrete"] = {
        main_product = "refined-concrete",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["repair-pack"] = {
        main_product = "repair-pack",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      roboport = {
        main_product = "roboport",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      rocket = {
        main_product = "rocket",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rocket-launcher"] = {
        main_product = "rocket-launcher",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rocket-silo"] = {
        main_product = "rocket-silo",
        results = {
          {
            amount = 10,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["rocket-turret"] = {
        main_product = "rocket-turret",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      shotgun = {
        main_product = "shotgun",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["shotgun-shell"] = {
        main_product = "shotgun-shell",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["slowdown-capsule"] = {
        main_product = "slowdown-capsule",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["small-electric-pole"] = {
        main_product = "small-electric-pole",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["small-lamp"] = {
        main_product = "small-lamp",
        results = {
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["solar-panel"] = {
        main_product = "solar-panel",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["solar-panel-equipment"] = {
        main_product = "solar-panel-equipment",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["space-platform-foundation"] = {
        main_product = "space-platform-foundation",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["space-platform-starter-pack"] = {
        main_product = "space-platform-starter-pack",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["space-science-pack"] = {
        main_product = "space-science-pack",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      splitter = {
        main_product = "splitter",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steam-engine"] = {
        main_product = "steam-engine",
        results = {
          {
            amount = 5,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steam-turbine"] = {
        main_product = "steam-turbine",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 4,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steel-chest"] = {
        main_product = "steel-chest",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steel-furnace"] = {
        main_product = "steel-furnace",
        results = {
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steel-plate"] = {
        main_product = "steel-plate",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["steel-plate-recycling"] = {
        main_product = "steel-plate",
        results = {
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["storage-tank"] = {
        main_product = "storage-tank",
        results = {
          {
            amount = 4,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["submachine-gun"] = {
        main_product = "submachine-gun",
        results = {
          {
            amount = 6,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      substation = {
        main_product = "substation",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 2,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["sulfuric-acid"] = {
        main_product = "sulfuric-acid",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      supercapacitor = {
        main_product = "supercapacitor",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      superconductor = {
        main_product = "superconductor",
        results = {
          {
            amount = 1,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "copper-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      tank = {
        main_product = "tank",
        results = {
          {
            amount = 4,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      teslagun = {
        main_product = "teslagun",
        results = {
          {
            amount = 3,
            name = "holmium-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      thruster = {
        main_product = "thruster",
        results = {
          {
            amount = 3,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["thruster-oxidizer"] = {
        main_product = "thruster-oxidizer",
        results = {
          {
            amount = 1,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["train-stop"] = {
        main_product = "train-stop",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          },
          {
            amount = 1,
            name = "steel-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["transport-belt"] = {
        main_product = "transport-belt",
        results = {
          {
            amount = 2,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["tungsten-carbide"] = {
        main_product = "tungsten-carbide",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["tungsten-ore-recycling"] = {
        main_product = "tungsten-ore",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["tungsten-plate"] = {
        main_product = "tungsten-plate",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["tungsten-plate-recycling"] = {
        main_product = "tungsten-plate",
        results = {
          {
            amount = 1,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["turbo-splitter"] = {
        main_product = "turbo-splitter",
        results = {
          {
            amount = 3,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["turbo-transport-belt"] = {
        main_product = "turbo-transport-belt",
        results = {
          {
            amount = 2,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["turbo-underground-belt"] = {
        main_product = "turbo-underground-belt",
        results = {
          {
            amount = 4,
            name = "tungsten-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["underground-belt"] = {
        main_product = "underground-belt",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      },
      ["uranium-fuel-cell"] = {
        main_product = "uranium-fuel-cell",
        results = {
          {
            amount = 3,
            name = "iron-scrap",
            probability = 0.23999999999999999,
            type = "item"
          }
        }
      }
    }
  },
  materials = {
    fluid = {
      "lithium"
    },
    solid = {
      "iron",
      "copper",
      "tungsten",
      "steel",
      "holmium",
      "brass",
      "bronze",
      "invar",
      "nitinol"
    },
    suffixes = {
      "-plate",
      "-ore",
      "-ingot",
      "-alloy",
      "-sheet",
      "-gear-wheel",
      "-cable"
    }
  },
  prototypes = {
    items = {
      ["copper-scrap"] = {
        drop_sound = {
          aggregation = {
            max_count = 1,
            remove = true
          },
          filename = "__base__/sound/item/metal-small-inventory-move.ogg",
          volume = 0.8
        },
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = {
              {
                b = 0.08235294117647058,
                g = 0.37647058823529411,
                r = 0.79607843137254894
              }
            }
          },
          {
            icon = "__base__/graphics/icons/copper-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          }
        },
        inventory_move_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        name = "copper-scrap",
        order = "is-[copper-scrap]",
        pick_sound = {
          aggregation = {
            max_count = 1,
            remove = true
          },
          filename = "__base__/sound/item/metal-small-inventory-pickup.ogg",
          volume = 0.8
        },
        pictures = {
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-1-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-2-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-3-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          }
        },
        stack_size = 200,
        subgroup = "raw-material",
        type = "item"
      },
      ["holmium-scrap"] = {
        default_import_location = "fulgora",
        drop_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = {
              1,
              0.95,
              0.9,
              1
            }
          },
          {
            icon = "__space-age__/graphics/icons/holmium-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          }
        },
        inventory_move_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        name = "holmium-scrap",
        order = "is-[holmium-scrap]",
        pick_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].pick_sound ]=],
        pictures = {
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-1-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-2-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-3-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          }
        },
        stack_size = 200,
        subgroup = "raw-material",
        type = "item"
      },
      ["iron-scrap"] = {
        drop_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = {
              {
                b = 0.55294117647058822,
                g = 0.5450980392156862,
                r = 0.5333333333333333
              }
            }
          },
          {
            icon = "__base__/graphics/icons/iron-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          }
        },
        inventory_move_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        name = "iron-scrap",
        order = "is-[iron-scrap]",
        pick_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].pick_sound ]=],
        pictures = {
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-1-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-2-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-3-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          }
        },
        stack_size = 200,
        subgroup = "raw-material",
        type = "item"
      },
      ["steel-scrap"] = {
        drop_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["iron-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__base__/graphics/icons/steel-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          }
        },
        inventory_move_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        name = "steel-scrap",
        order = "is-[steel-scrap]",
        pick_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].pick_sound ]=],
        pictures = {
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-1-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-2-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-3-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          }
        },
        stack_size = 200,
        subgroup = "raw-material",
        type = "item"
      },
      ["tungsten-scrap"] = {
        default_import_location = "vulcanus",
        drop_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = {
              a = 0.8,
              b = 177,
              g = 85,
              r = 101
            }
          },
          {
            icon = "__space-age__/graphics/icons/tungsten-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          }
        },
        inventory_move_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].drop_sound ]=],
        name = "tungsten-scrap",
        order = "is-[tungsten-scrap]",
        pick_sound = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].pick_sound ]=],
        pictures = {
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-1-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-2-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          },
          {
            filename = "__Ingredient_Scrap__/graphics/icons/scrap-3-64.png",
            scale = 0.5,
            shift = {
              0,
              0
            },
            size = 64
          }
        },
        stack_size = 200,
        subgroup = "raw-material",
        type = "item"
      }
    },
    recipes = {
      ["recycle-copper-scrap"] = {
        allow_as_intermediate = false,
        always_show_products = true,
        categories = {
          "yis-recycle-to-item"
        },
        enabled = false,
        hide_from_player_crafting = true,
        icons = {
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].icons[1] ]=],
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].icons[2] ]=],
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 0.25
          }
        },
        ingredients = {
          {
            amount = 1,
            name = "copper-scrap",
            type = "item"
          }
        },
        localised_name = {
          "recipe-name.recycle-copper-scrap"
        },
        name = "recycle-copper-scrap",
        order = "is-[recycle-copper-scrap]",
        results = {
          {
            amount = 1,
            name = "copper-plate",
            type = "item"
          }
        },
        subgroup = "raw-material",
        type = "recipe"
      },
      ["recycle-holmium-scrap"] = {
        allow_as_intermediate = false,
        always_show_products = true,
        categories = {
          "yis-recycle-to-item"
        },
        enabled = false,
        hide_from_player_crafting = true,
        icons = {
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["holmium-scrap"].icons[1] ]=],
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["holmium-scrap"].icons[2] ]=],
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 0.25
          }
        },
        ingredients = {
          {
            amount = 1,
            name = "holmium-scrap",
            type = "item"
          }
        },
        localised_name = {
          "recipe-name.recycle-holmium-scrap"
        },
        name = "recycle-holmium-scrap",
        order = "is-[recycle-holmium-scrap]",
        results = {
          {
            amount = 1,
            name = "holmium-plate",
            type = "item"
          }
        },
        subgroup = "raw-material",
        type = "recipe"
      },
      ["recycle-iron-scrap"] = {
        allow_as_intermediate = false,
        always_show_products = true,
        categories = {
          "yis-recycle-to-item"
        },
        enabled = false,
        hide_from_player_crafting = true,
        icons = {
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["iron-scrap"].icons[1] ]=],
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["iron-scrap"].icons[2] ]=],
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 0.25
          }
        },
        ingredients = {
          {
            amount = 1,
            name = "iron-scrap",
            type = "item"
          }
        },
        localised_name = {
          "recipe-name.recycle-iron-scrap"
        },
        name = "recycle-iron-scrap",
        order = "is-[recycle-iron-scrap]",
        results = {
          {
            amount = 1,
            name = "iron-plate",
            type = "item"
          }
        },
        subgroup = "raw-material",
        type = "recipe"
      },
      ["recycle-steel-scrap"] = {
        allow_as_intermediate = false,
        always_show_products = true,
        categories = {
          "yis-recycle-to-item"
        },
        enabled = false,
        hide_from_player_crafting = true,
        icons = {
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["steel-scrap"].icons[1] ]=],
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["steel-scrap"].icons[2] ]=],
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 0.25
          }
        },
        ingredients = {
          {
            amount = 1,
            name = "steel-scrap",
            type = "item"
          }
        },
        localised_name = {
          "recipe-name.recycle-steel-scrap"
        },
        name = "recycle-steel-scrap",
        order = "is-[recycle-steel-scrap]",
        results = {
          {
            amount = 1,
            name = "steel-plate",
            type = "item"
          }
        },
        subgroup = "raw-material",
        type = "recipe"
      },
      ["recycle-tungsten-scrap"] = {
        allow_as_intermediate = false,
        always_show_products = true,
        categories = {
          "yis-recycle-to-item"
        },
        enabled = false,
        hide_from_player_crafting = true,
        icons = {
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["tungsten-scrap"].icons[1] ]=],
          "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["tungsten-scrap"].icons[2] ]=],
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 0.25
          }
        },
        ingredients = {
          {
            amount = 1,
            name = "tungsten-scrap",
            type = "item"
          }
        },
        localised_name = {
          "recipe-name.recycle-tungsten-scrap"
        },
        name = "recycle-tungsten-scrap",
        order = "is-[recycle-tungsten-scrap]",
        results = {
          {
            amount = 1,
            name = "tungsten-plate",
            type = "item"
          }
        },
        subgroup = "raw-material",
        type = "recipe"
      }
    },
    technology = {
      ["recycle-copper-scrap"] = {
        effects = {
          {
            recipe = "recycle-copper-scrap",
            type = "unlock-recipe"
          }
        },
        enabled = false,
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["copper-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__base__/graphics/icons/copper-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          },
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 1
          }
        },
        name = "recycle-copper-scrap",
        research_trigger = {
          count = 1,
          item = "copper-scrap",
          type = "craft-item"
        },
        type = "technology",
        visible_when_disabled = false
      },
      ["recycle-holmium-scrap"] = {
        effects = {
          {
            recipe = "recycle-holmium-scrap",
            type = "unlock-recipe"
          }
        },
        enabled = false,
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["holmium-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__space-age__/graphics/icons/holmium-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          },
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 1
          }
        },
        name = "recycle-holmium-scrap",
        research_trigger = {
          count = 1,
          item = "holmium-scrap",
          type = "craft-item"
        },
        type = "technology",
        visible_when_disabled = false
      },
      ["recycle-iron-scrap"] = {
        effects = {
          {
            recipe = "recycle-iron-scrap",
            type = "unlock-recipe"
          }
        },
        enabled = false,
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["iron-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__base__/graphics/icons/iron-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          },
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 1
          }
        },
        name = "recycle-iron-scrap",
        research_trigger = {
          count = 1,
          item = "iron-scrap",
          type = "craft-item"
        },
        type = "technology",
        visible_when_disabled = false
      },
      ["recycle-steel-scrap"] = {
        effects = {
          {
            recipe = "recycle-steel-scrap",
            type = "unlock-recipe"
          }
        },
        enabled = false,
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["iron-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__base__/graphics/icons/steel-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          },
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 1
          }
        },
        name = "recycle-steel-scrap",
        research_trigger = {
          count = 1,
          item = "steel-scrap",
          type = "craft-item"
        },
        type = "technology",
        visible_when_disabled = false
      },
      ["recycle-tungsten-scrap"] = {
        effects = {
          {
            recipe = "recycle-tungsten-scrap",
            type = "unlock-recipe"
          }
        },
        enabled = false,
        icons = {
          {
            icon = "__Ingredient_Scrap__/graphics/icons/scrap.png",
            icon_size = 64,
            tint = "SERPENT PLACEHOLDER" --[=[ ref [""].prototypes.items["tungsten-scrap"].icons[1].tint ]=]
          },
          {
            icon = "__space-age__/graphics/icons/tungsten-plate.png",
            scale = 0.25,
            shift = {
              -8,
              -8
            },
            size = 64
          },
          {
            icon = "__Ingredient_Scrap__/graphics/icons/recycle.png",
            icon_size = 256,
            scale = 1
          }
        },
        name = "recycle-tungsten-scrap",
        research_trigger = {
          count = 1,
          item = "tungsten-scrap",
          type = "craft-item"
        },
        type = "technology",
        visible_when_disabled = false
      }
    }
  }
}