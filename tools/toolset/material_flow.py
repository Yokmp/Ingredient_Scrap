...
def create_dump(
    ...
    production_path = run_tests.production_flow_path(factorio_exe)
    data_table_path = run_tests.data_table_path(factorio_exe)
    report_path = run_tests.report_path(factorio_exe)

    for path in (save_path, flow_path, production_path, data_table_path, report_path):
        try:
            path.unlink()
    ...
    if production_path.exists():
        run_tests.enrich_material_flow_metadata(
            factorio_exe,
            production_path,
            run_tests.production_flow_state_path(factorio_exe),
        )
    profiled_flow_path = copy_profiled_dump(flow_path, mod_profile)
    profiled_production_path = copy_profiled_dump(production_path, mod_profile)
    profiled_state_path = copy_profiled_dump(run_tests.material_flow_state_path(factorio_exe), mod_profile)
    profiled_production_state_path = copy_profiled_dump(run_tests.production_flow_state_path(factorio_exe), mod_profile)
    ...
    if profiled_flow_path:
        print(f"Profile flow:  {profiled_flow_path}")
    if profiled_production_path:
        print(f"Profile prod:  {profiled_production_path}")
    if profiled_state_path:
        print(f"Profile state: {profiled_state_path}")
    if profiled_production_state_path:
        print(f"Profile prod state: {profiled_production_state_path}")
    print(f"Icon assets:   {run_tests.icon_assets_path(factorio_exe)}")
    return 0


def viewer_url(factorio_exe: Path, mod_profile: str | None = None) -> str:
    viewer = TOOL_DIR / "json-tree-viewer.html"
    flow_path = profiled_path(run_tests.material_flow_path(factorio_exe), mod_profile)
    state_path = profiled_path(run_tests.material_flow_state_path(factorio_exe), mod_profile)
...
def main() -> int:
    ...
    try:
        ...
        status = create_dump(factorio_exe, args.dump_profile, args.mod_profile, extra_args)
        if status == 0 and args.open_viewer:
            webbrowser.open(viewer_url(factorio_exe, args.mod_profile))
        return status
    finally:
        if not args.no_debug_setting:
