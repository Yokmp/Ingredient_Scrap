...
function yokmods.ingredient_scrap.get_scrap_amount(data_table, ingredient, recipe, scrap_type)
  ...
  data_table.inserts[recipe.name][scrap_name] = data_table.inserts[recipe.name][scrap_name] or {}

  -- set amounts
  if data_table.inserts[recipe.name][scrap_name].amount_min then
    if not ISsettings.amount_range then
      data_table.inserts[recipe.name][scrap_name].amount =
          data_table.inserts[recipe.name][scrap_name].amount + amount
    end
    data_table.inserts[recipe.name][scrap_name].amount_min =
        data_table.inserts[recipe.name][scrap_name].amount_min + min
    data_table.inserts[recipe.name][scrap_name].amount_max =
        data_table.inserts[recipe.name][scrap_name].amount_max + max
  else
    data_table.inserts[recipe.name][scrap_name] = {
      type = "item",
      name = scrap_name,
      amount_max = max,
      amount_min = min,
    ...
    if not ISsettings.amount_range then data_table.inserts[recipe.name][scrap_name].amount = amount end
  end
end
...
function yokmods.ingredient_scrap.make_scrap_item(scrap_defines)
  ...
  if data.raw.item[scrap_defines.name].icon then
    scrap_item.icons[2] = icon_scale_and_shift({ icon = data.raw.item[scrap_defines.name].icon, icon_size = data.raw
    .item[scrap_defines.name].icon_size })
  else
